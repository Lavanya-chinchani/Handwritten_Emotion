...
def extractLines(img):
    ...
    for i, line in enumerate(fineLines):
        if isinstance(line[0], int) and isinstance(line[1], int) and line[0] < line[1]:
            segment = hpList[line[0]:line[1]]
        for j, sum in enumerate(segment):
...
def extractWords(image, lines):
    ...
    for i, line in enumerate(lines):
    # Ensure that line[0], line[1], and width are integers
        start_index = int(line[0])
        end_index = int(line[1])
        width_int = int(width)

    # Use the integer values as slice indices
        extract = thresh[start_index:end_index, 0:width_int]  # y1:y2, x1:x2
...
def extractSlant(img, words):
    ...
    # Ensure that word[0], word[1], word[2], and word[3] are integers
            start_index = int(word[0])
            end_index = int(word[1])
            start_column = int(word[2])
            end_column = int(word[3])

    # Use the integer values as slice indices
            original = thresh[start_index:end_index, start_column:end_column]  # y1:y2, x1:x2
  # y1:y2, x1:x2
    ...
    if (max_index == 0):
        ...
        if s_function[3] != 0 and s_function[5]!=0:
...
##        
        type = 'e' #input("Press enter if okay, else enter c to change: ")
